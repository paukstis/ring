...
def main(fn, test_type, padding_alg):
    ...
    for case in parse(fn, last_field):
        if case['SHAAlg'] == 'SHA224':
            ...
            if DEBUG:
                print "Skipping due to use of SHA224"
            continue

        if case['SHAAlg'] == 'SHA1' and padding_alg == 'PSS':
            # SHA1 with PSS not supported in *ring.
            if DEBUG:
                print "Skipping due to use of SHA1 and PSS."
            continue
        ...
        if padding_alg == "PSS":
            if len(case['SaltVal']) * 2 != DIGEST_OUTPUT_LENGTHS[case['SHAAlg']]:
                if DEBUG:
                    print "Skipping due to incorrect salt length."
                continue
        ...
        if n.bit_length() < 2048:
            if DEBUG:
                print "Skipping due to modulus length (too small)."
            continue

        if n.bit_length() > 4096:
            if DEBUG:
                print "Skipping due to modulus length (too large)."
            continue

        # Can only support moduli which are a whole number of bytes
        if n.bit_length() % 8 != 0:
            if DEBUG:
                print "Skipping due to modulus (not a whole number of bytes)."
            continue
    ...
    sys.stderr.write("%d test cases output.\n" % num_cases)
    sys.stderr.flush()
